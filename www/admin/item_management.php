                ...
                <tr>
                    ...
                    <td>
                        <a href="javascript:void(0);" class="action-btn" 
                            onclick="openEditModal(<?php echo $item['id']; ?>, '<?php echo addslashes($item['title']); ?>', '<?php echo addslashes($item['description']); ?>', '<?php echo addslashes($item['type']); ?>', '<?php echo addslashes($item['location']); ?>', '<?php echo addslashes($item['image']); ?>')">
...
function openEditModal(itemId, title, description, type, location, image) {
    ...
    // If there's an image, display it in the form
    if (image) {
        document.getElementById("currentImage").src = "../uploads/" + image;
    } else {
